...
async function boot(user){
  state.user=user;
  const msg = m => { if ($('#authMessage')) $('#authMessage').textContent=m; };
  const fail = (where, err) => {
    console.error(where, err);
    msg(`تعذر تحضير جلسة المدرس — ${where}: ${err?.message || err}`);
    $('#loginForm')?.classList.add('hidden');
    $('#authOverlay')?.classList.remove('hide');
  };
  try {
    msg('تم تسجيل الدخول. جارٍ قراءة بيانات المدرس...');
    let p=null;
    try {
      const u=await getDoc(doc(db,'users',user.uid));
      if(u.exists()) p={id:u.id,...u.data()};
      else console.warn('users/{uid} غير موجود:', user.uid);
    } catch(err) { console.warn('تعذر قراءة users/{uid}:',err); }

    msg('جارٍ البحث عن ملف المدرس في teachers...');
    try {
      const email=(user.email||'').trim();
      if(!email) throw new Error('حساب Firebase لا يحتوي على بريد إلكتروني.');
      let tSnap=await getDocs(query(collection(db,'teachers'),where('email','==',email)));
      if(!tSnap.empty) state.teacher={id:tSnap.docs[0].id,...tSnap.docs[0].data()};
    } catch(err) { fail('قراءة مجموعة teachers',err); return; }

    if(!p && state.teacher) p={...state.teacher,role:'teacher',name:state.teacher.name||state.teacher.fullName||user.displayName||'المدرس',email:user.email||state.teacher.email||''};
    if(!p) throw new Error(`لم يتم العثور على بيانات المدرس. UID: ${user.uid} | البريد: ${user.email||'غير موجود'}`);
    const role=String(p.role||'').trim().toLowerCase();
    if(!['teacher','admin'].includes(role)) throw new Error(`صلاحية الحساب غير صحيحة. الدور الحالي: ${p.role||'غير محدد'}؛ يجب أن يكون teacher أو admin.`);
    ...
    // أخفِ شاشة التحقق قبل تحميل بيانات لوحة التحكم حتى لا تبقى عالقة بسبب خطأ في البيانات.
    $('#authOverlay').classList.add('hide');
    $('#connectionStatus').textContent='متصل';
    setupFilters();
    try { await load(); }
    catch(err) {
      console.error('فشل تحميل بيانات لوحة المدرس:',err);
      $('#connectionStatus').textContent='متصل - خطأ في البيانات';
      toast(`تم فتح اللوحة، لكن تعذر تحميل البيانات: ${err?.message||err}`);
    }
  } catch(err) { fail('التحقق من حساب المدرس',err); }
}

$('#todayDate').textContent=new Date().toLocaleDateString('ar-IQ',{weekday:'long',year:'numeric',month:'long',day:'numeric'});
$('#loginForm').onsubmit=async e=>{e.preventDefault();$('#loginError').textContent='';try{await signInWithEmailAndPassword(auth,$('#loginEmail').value.trim(),$('#loginPassword').value)}catch(err){console.error('Teacher login failed:',err);$('#loginError').textContent=`فشل تسجيل الدخول: ${err?.message||'بيانات الدخول غير صحيحة أو الحساب غير مفعل.'}`}};
onAuthStateChanged(auth,async user=>{try{if(!user){$('#authMessage').textContent='سجّل الدخول بحساب المدرس.';$('#loginForm').classList.remove('hidden');$('#authOverlay').classList.remove('hide');return}await boot(user)}catch(err){console.error('Unhandled auth error:',err);$('#authMessage').textContent=`خطأ غير متوقع: ${err?.message||err}`;$('#authOverlay').classList.remove('hide')}});
