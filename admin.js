...
function newScheduleId(){ return (crypto.randomUUID ? crypto.randomUUID() : `sch_${Date.now()}_${Math.random().toString(36).slice(2,8)}`); }
function scheduleMetaId(classId,scheduleId){ return `meta__${classId}__${scheduleId}`; }
function scheduleCellId(classId,scheduleId,day,period){ return `${classId}__${scheduleId}__${encodeURIComponent(day)}__${period}`; }
function scheduleMetas(classId){
  return state.schedules.filter(s=>s.recordType==="meta" && s.classId===classId && s.deleted!==true)
    .sort((a,b)=>String(a.createdAt?.toDate?.()||a.createdAt||"").localeCompare(String(b.createdAt?.toDate?.()||b.createdAt||"")));
}
function currentScheduleId(){ return $("#scheduleNameFilter")?.value||""; }
function populateScheduleNames(){
  const classId=$("#scheduleClassFilter")?.value||"";
  const select=$("#scheduleNameFilter"); if(!select)return;
  const old=select.value;
  const metas=scheduleMetas(classId);
  select.innerHTML=`<option value="">اختر الجدول</option>`+metas.map(m=>`<option value="${escapeHTML(m.scheduleId)}">${escapeHTML(m.scheduleName||"جدول أسبوعي")}</option>`).join("");
  if(metas.some(m=>m.scheduleId===old)) select.value=old;
  else if(metas.length===1) select.value=metas[0].scheduleId;
}
function scheduleGridData(classId,scheduleId){
  const map={};
  state.schedules.filter(s=>s.recordType!=="meta" && s.classId===classId && s.scheduleId===scheduleId && s.deleted!==true)
    .forEach(s=>{map[`${s.day}__${String(s.period)}`]=s;});
  return map;
}
...
function renderSchedules(){
  ...
  if(selected && currentScheduleId()) renderScheduleGrid();
  else $("#scheduleGridWrap").innerHTML='<div class="empty">اختر الصف والشعبة ثم اختر الجدول.</div>';
}
...
function renderScheduleGrid(){
  ...
  if(!scheduleId)return toast("اختر الجدول أولاً");
  const meta=scheduleMetas(classId).find(x=>x.scheduleId===scheduleId);
  const count=Number($("#schedulePeriodCount").value||meta?.periodCount||8);
  const map=scheduleGridData(classId,scheduleId);
  ...
  let html=`<div class="schedule-current-title"><strong>${escapeHTML(meta?.scheduleName||"الجدول الأسبوعي")}</strong><span>${escapeHTML(meta?.className||"")} / ${escapeHTML(meta?.sectionName||"")}</span></div>`;
  html+='<table class="weekly-schedule"><thead><tr><th class="period-head">الحصة</th>';
  html+=SCHEDULE_DAYS.map(d=>`<th>${d}</th>`).join('');
  html+='</tr></thead><tbody>';
  for(let p=1;p<=count;p++){
    html+=`<tr><th class="period-head">${p}</th>`;
    for(const day of SCHEDULE_DAYS){
      const sc=map[`${day}__${p}`]||{};
      html+=`<td class="schedule-cell">
        <input class="schedule-subject" data-day="${escapeHTML(day)}" data-period="${p}" value="${escapeHTML(sc.subject||"")}" placeholder="المادة">
...
$("#scheduleClassFilter").onchange=()=>{ populateScheduleNames(); $("#scheduleGridWrap").innerHTML='<div class="empty">اختر الجدول المطلوب أو أضف جدولًا جديدًا.</div>'; };
$("#scheduleNameFilter").onchange=()=>{ if(currentScheduleId()) renderScheduleGrid(); };
$("#loadScheduleGridBtn").onclick=renderScheduleGrid;
$("#schedulePeriodCount").onchange=()=>{if($("#scheduleClassFilter").value && currentScheduleId())renderScheduleGrid();};

$("#addWeeklyScheduleBtn").onclick=()=>{
  const opts=state.classes.map(c=>`<option value="${escapeHTML(c.id)}">${escapeHTML(c.name||c.className||"")} / ${escapeHTML(c.sectionName||c.section||"")}</option>`).join("");
  showModal(`<h2>إضافة جدول أسبوعي</h2><div class="form-grid">
    <label>الصف والشعبة<select id="newScheduleClass">${opts}</select></label>
    <label>اسم الجدول<input id="newScheduleName" placeholder="مثال: الجدول الأساسي"></label>
    <label>عدد الحصص<select id="newSchedulePeriods"><option value="6">6</option><option value="7">7</option><option value="8" selected>8</option><option value="9">9</option></select></label>
    <button class="primary" id="createWeeklySchedule">إنشاء الجدول</button>
  </div>`);
  $("#createWeeklySchedule").onclick=async()=>{
    const classId=$("#newScheduleClass").value;
    const c=state.classes.find(x=>x.id===classId);
    const scheduleName=$("#newScheduleName").value.trim();
    const periodCount=Number($("#newSchedulePeriods").value||8);
    if(!c||!scheduleName)return toast("أدخل اسم الجدول واختر الصف والشعبة");
    const scheduleId=newScheduleId();
    try{
      await setDoc(doc(db,"schedules",scheduleMetaId(classId,scheduleId)),{
        recordType:"meta",scheduleId,scheduleName,periodCount,classId,
        className:c.name||c.className||"",sectionId:c.sectionId||c.id,
        sectionName:c.sectionName||c.section||"",deleted:false,createdAt:serverTimestamp(),updatedAt:serverTimestamp()
      });
      closeModal(); await loadData();
      $("#scheduleClassFilter").value=classId; populateScheduleNames(); $("#scheduleNameFilter").value=scheduleId; $("#schedulePeriodCount").value=String(periodCount); renderScheduleGrid();
      toast("تم إنشاء الجدول ويمكنك تعبئته الآن");
    }catch(e){console.error(e);toast("تعذر إنشاء الجدول");}
  };
};

$("#saveScheduleGridBtn").onclick=async()=>{
  const classId=$("#scheduleClassFilter").value, scheduleId=currentScheduleId();
  if(!classId)return toast("اختر الصف والشعبة أولاً");
  if(!scheduleId)return toast("اختر الجدول أولاً");
  const c=state.classes.find(x=>x.id===classId); if(!c)return;
  const meta=scheduleMetas(classId).find(x=>x.scheduleId===scheduleId); if(!meta)return toast("لم يتم العثور على الجدول");
  const count=Number($("#schedulePeriodCount").value||meta.periodCount||8);
  const old=state.schedules.filter(s=>s.recordType!=="meta" && s.classId===classId && s.scheduleId===scheduleId && s.deleted!==true);
  ...
  try{
    ...
    for(let p=1;p<=count;p++) for(const day of SCHEDULE_DAYS){
      ...
      if(!subject && !teacherId){ if(oldDoc) ops.push(updateDoc(doc(db,"schedules",oldDoc.id),{deleted:true,deletedAt:serverTimestamp()})); }
      else{
        const data={recordType:"cell",scheduleId,scheduleName:meta.scheduleName,day,period:String(p),classId,className:c.name||c.className||"",sectionId:c.sectionId||c.id,sectionName:c.sectionName||c.section||"",subject,teacherId:teacherId||"",teacherName:teacher?.name||"",deleted:false,updatedAt:serverTimestamp()};
        if(oldDoc) ops.push(updateDoc(doc(db,"schedules",oldDoc.id),data));
        else ops.push(setDoc(doc(db,"schedules",scheduleCellId(classId,scheduleId,day,p)),{...data,createdAt:serverTimestamp()}));
      }
    }
    ops.push(updateDoc(doc(db,"schedules",scheduleMetaId(classId,scheduleId)),{periodCount:count,updatedAt:serverTimestamp()}));
    await Promise.all(ops);
    toast("تم حفظ الجدول بالكامل"); await loadData(); $("#scheduleClassFilter").value=classId; populateScheduleNames(); $("#scheduleNameFilter").value=scheduleId; renderScheduleGrid();
  }catch(e){console.error(e);toast("تعذر حفظ الجدول");}
};

$("#deleteWeeklyScheduleBtn").onclick=async()=>{
  const classId=$("#scheduleClassFilter").value, scheduleId=currentScheduleId();
  if(!classId||!scheduleId)return toast("اختر الجدول الذي تريد حذفه");
  const meta=scheduleMetas(classId).find(x=>x.scheduleId===scheduleId);
  if(!confirm(`هل تريد حذف الجدول «${meta?.scheduleName||""}» بالكامل؟`))return;
  try{
    const docs=state.schedules.filter(s=>(s.recordType==="meta" && s.id===scheduleMetaId(classId,scheduleId)) || (s.recordType!=="meta" && s.classId===classId && s.scheduleId===scheduleId && s.deleted!==true));
    await Promise.all(docs.map(s=>updateDoc(doc(db,"schedules",s.id),{deleted:true,deletedAt:serverTimestamp()})));
    await loadData(); $("#scheduleClassFilter").value=classId; populateScheduleNames(); $("#scheduleGridWrap").innerHTML='<div class="empty">تم حذف الجدول. اختر جدولًا آخر أو أضف جدولًا جديدًا.</div>'; toast("تم حذف الجدول");
  }catch(e){console.error(e);toast("تعذر حذف الجدول");}
};
